-- ============================================================================
-- FULL REPORT - Combines SurfaceTreatments, RepairReport, GM, and POpsServiceList
-- ============================================================================
-- Each query runs separately and produces its own result set
-- ============================================================================

-- ============================================================================
-- QUERY 1: SurfaceTreatments Report
-- ============================================================================
...
-- NEW: individual ST service IDs per job (using work_orders.id)
ST_ServiceList AS (
    SELECT
        job_id,
        MAX(CASE WHEN rn = 1 THEN CAST(service_row_id AS VARCHAR(20)) END) AS ServiceID1,
        MAX(CASE WHEN rn = 2 THEN CAST(service_row_id AS VARCHAR(20)) END) AS ServiceID2,
        MAX(CASE WHEN rn = 3 THEN CAST(service_row_id AS VARCHAR(20)) END) AS ServiceID3
    FROM (
        SELECT DISTINCT
            job_id,
            service_row_id,
            ROW_NUMBER() OVER (PARTITION BY job_id ORDER BY service_row_id) AS rn
        FROM ST_Base
...
SurfaceAgg AS (
    SELECT
        ...
        CASE 
            ...
            ELSE ''
        END                                            AS [YR CMPLT],
        sl.ServiceID1                                 AS [Service ID1],
        sl.ServiceID2                                 AS [Service ID2],
        sl.ServiceID3                                 AS [Service ID3],
    ...
    GROUP BY b.job_id, sl.ServiceID1, sl.ServiceID2, sl.ServiceID3, sn.ServiceNames
),
...
SELECT
    ...
    sa.TCD                  AS [ST-TCD],
    sa.[POps Costing Year]  AS [POps Costing Year],
    ''                      AS [NS INV YR],
    sa.[Service ID1]        AS [ST-Service ID1],
    ''                      AS [ST-Rev 1],
    sa.[Service ID2]        AS [ST-Service ID2],
    ''                      AS [ST-Rev 2],
    sa.[Service ID3]        AS [ST-Service ID3],
    ''                      AS [ST - Rev 3],
    ...
    -- R: Repair (cat 2 & 3) – ONLY these two
